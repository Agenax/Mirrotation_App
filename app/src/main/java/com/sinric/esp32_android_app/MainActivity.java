...
public class MainActivity extends AppCompatActivity implements OnBluetoothDeviceClickedListener,DRecieved_2{
    private final int REQUEST_PERMISSION_ACCESS_FINE_LOCATION=1;
    //private Activity_2 A2 = new Activity_2();
    ...
    private ScanCallback mBluetoothScanCallBackNG;
        ...
        handler1.postDelayed(new Runnable() {
            ...
            public void run() {
                if (mBluetoothLeService != null &&
                        mConnectionState.equals(BluetoothLeService.ACTION_GATT_CONNECTED) && data != null) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (this.checkSelfPermission(Manifest.permission.BLUETOOTH_CONNECT)
                    ...
                    this.checkSelfPermission(Manifest.permission.BLUETOOTH_ADVERTISE)
                            != PackageManager.PERMISSION_GRANTED) {
                requestPermissions( new String[]{Manifest.permission.BLUETOOTH_CONNECT, Manifest.permission.BLUETOOTH_SCAN, Manifest.permission.BLUETOOTH_ADMIN , Manifest.permission.BLUETOOTH , Manifest.permission.BLUETOOTH_ADVERTISE}, REQUEST_CODE_BLUETOOTH);
            }
        }
    }
    ...
    private void scanLeDevice(boolean enable) {
        if (enable) {
            ...
            BluetoothScan.startScan(true, mBluetoothScanCallBackNG);
        } else {
            swipeRefresh.setRefreshing(false);
            BluetoothScan.stopScan();
        }
    ...
    private BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothLeService.ACTION_GATT_CONNECTED.equals(action)) {
                ...
                Activity_opt aClass = new Activity_opt();
                active = aClass.getActive();
                if (active) {
                    //Activity_opt options = new Activity_opt();
                    //options.DataRecieved(stringBuilder.toString());
                    //options.DataRecieved();
                    //showMsg("Got string : " + new String(data));
                }
                if (data != null && data.length > 0) {
                    ...
                    myApp.setRecieved(new String(data));
                    //Activity_opt aClass = new Activity_opt();
                    active = aClass.getActive();
                    if (active) {
                        //Activity_opt options = new Activity_opt();
                        //options.DataRecieved(stringBuilder.toString());
                        //options.DataRecieved(new String(data));
                        //        showMsg("Got string : " + new String(data));

                        //((MyApplication) MyApplication.context()).setRecieved(new String(data));
                       // MyApplication.Global.svar1 = "hey";
                        //showMsg(MyApplication.Global.svar1);
                    }
    ...
    public void btSendBytes(byte[] data) {
        Log.i("MainActivity","Sending bytes");
        if (mBluetoothLeService != null /*&&
    ...
    private class MyBluetoothScanCallBack implements BluetoothScan.BluetoothScanCallBack {
        ...
        public void onLeScanInitSuccess(int successCode) {
            ...
            switch (successCode) {
                ...
                case BluetoothScan.SCAN_NEED_ENADLE :
                    ...
                    try{startActivityForResult(enableBtIntent, REQUEST_ENABLE_BT);}
        ...
        public void onLeScanResult(BluetoothDevice device, int rssi, byte[] scanRecord) {
            if(!mBluetoothDeviceList.contains(device) && device != null) {
                ...
                String s = "nul";
                try{s = device.getName();}
                catch(SecurityException e){}
                Log.i("MainActivity","notifyDataSetChanged() " + "BluetoothName :　" + s +
